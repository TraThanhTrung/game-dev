...
/// <summary>
/// UI component for displaying a single enemy type in the result list.
/// </summary>
public class EnemyListItem : MonoBehaviour
{
    ...
    public void Initialize(GameResult.EnemyTypeInfo enemyInfo)
    {
        if (enemyInfo == null)
        {
            Debug.LogWarning("[EnemyListItem] Enemy info is null");
            return;
        }

        // Display enemy name from server, fallback to capitalized typeId if name is empty
